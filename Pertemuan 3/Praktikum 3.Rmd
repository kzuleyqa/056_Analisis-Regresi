...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\Asus Vivobook\\OneDrive\\Documents\\Kuliah\\Semester 4\\ANREG\\Dataset FIX Anreg.xlsx")
head(dt)
```

```{r}
y <- dt$"Persentase Rumah Tangga yang Pernah Mengakses Internet dalam 3 Bulan Terakhir Menurut Provinsi"
x <- dt$"Banyaknya Desa/Kelurahan yang Memiliki Menara Base Transceiver Station (BTS) menurut Provinsi"
n <- length(dt)
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Persentase Rumah Tangga yang Pernah Mengakses Internet dalam 3 Bulan Terakhir Menurut Provinsi)

$$E[\hat{Y}] = \hat{Y} = 79.76 + 0.0003748 X$$
...
text(30, 220, "Y_duga", adj = c(-0.1, 1.5), col = "green", cex = 0.8)
abline(h=y.bar, col="hotpink")
text(31, 185, "Y_bar", adj = c(-0.1, 1.5), col = "hotpink", cex = 0.8)
```
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 7.0 dan JKG model bernilai 3251.5

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 101.6088 Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{101.6088} \approx 10.08012$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 79.75503 dan 0.0003747635. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [ 69.09233 ,  90.41773 ] dan untuk parameter $β_1$ adalah [ -0.003295038 ,  0.004044565 ]
...
Misalkan kita ingin menduga nilai rataan (harapan) amatan ketika nilai x=300. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
amatan.diduga <- data.frame(x=300)
predict(model, amatan.diduga, interval = "confidence")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=300 adalah 79.86746. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=300 berada dalam selang 75.5671 hingga 84.16782.
...
Misalkan kita ingin menduga nilai individu amatan ketika nilai x=300. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
predict(model, amatan.diduga, interval = "prediction")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=300 adalah 79.86746. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=300 berada dalam selang 58.88942 hingga 100.8455.